...
export default function Edital001Page() {
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      <div>
        <h1 className="text-3xl font-bold">Edital Nº 001/2026 — SMUL/OUCAB</h1>
      ...
      <Card>
        ...
        <CardContent>
          <a
            href={`${BASE_PATH}/editais/Edital_N__001_2026_SMUL_OUCAB.pdf`}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-3 p-3 rounded-md border hover:bg-muted transition-colors"
          >
            <FileText className="w-5 h-5 text-red-500 flex-shrink-0" />
            <span className="flex-1 font-medium">Edital Nº 001/2026 — SMUL/OUCAB</span>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {ANEXOS.map(({ nome, arquivo }) => (
            <a
              key={arquivo}
              href={`${BASE_PATH}/editais/anexos/1/${arquivo}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-3 p-3 rounded-md border hover:bg-muted transition-colors"
            >
