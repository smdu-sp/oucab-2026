        ...
        <div className="relative">
          <Input
            ...
            onChange={(e) => {
              ...
              setValue(`${prefix}.tituloEleitor` as any, v);
              setTituloValido(null);
              if (v.length === 12) {
                if (validarTituloEleitor(v)) {
                  clearErrors(`${prefix}.tituloEleitor` as any);
                  setTituloValido(true);
                } else {
                  setError(`${prefix}.tituloEleitor` as any, { type: "manual", message: "Título de eleitor inválido" });
            ...
            maxLength={12}
            className={cn(erros.tituloEleitor && "border-red-500", tituloValido === true && "border-green-500")}
          />
          {tituloValido === true && <div className="absolute right-3 top-1/2 -translate-y-1/2"><CheckCircle className="w-4 h-4 text-green-500" /></div>}
          {tituloValido === false && !erros.tituloEleitor && <div className="absolute right-3 top-1/2 -translate-y-1/2"><AlertCircle className="w-4 h-4 text-red-500" /></div>}
        </div>
