...
const CACHE_KEY = "oucab:inscricao:rascunho";

type Rascunho = { step: number; valores: Partial<FormularioInscricaoData> };

function salvarRascunho(step: number, valores: Partial<FormularioInscricaoData>) {
  try {
    const serializavel = JSON.parse(
      JSON.stringify(valores, (_k, v) => (v instanceof File ? undefined : v)),
    );
...
export default function FormularioInscricao() {
  ...
  const [rascunhoBannerVisivel, setRascunhoBannerVisivel] = useState(false);
  ...
  useEffect(() => {
    ...
    if (r && r.step > 1) setRascunhoBannerVisivel(true);
  }, []);

  // Auto-salvar rascunho a cada mudança de formulário
  useEffect(() => {
    const subscription = watch((valores) => {
      if (debounceRef.current) clearTimeout(debounceRef.current);
      debounceRef.current = setTimeout(() => {
        salvarRascunho(etapaAtual, valores as Partial<FormularioInscricaoData>);
      }, 600);
    });
    return () => {
  ...
  const restaurarRascunho = () => {
    const r = carregarRascunho();
    if (!r) return;
    methods.reset(r.valores as FormularioInscricaoData);
    setEtapaAtual(r.step);
    setEtapasCompletas(Array.from({ length: r.step - 1 }, (_, i) => i + 1));
    setRascunhoBannerVisivel(false);
  };
  ...
  const onSubmit = async (data: FormularioInscricaoData) => {
    ...
    try {
      ...
      limparRascunho();
      router.push("/oucab/agradecimento");
    } catch (error) {
            ...
            <Alert className="mb-4 border-amber-400 bg-amber-50 dark:bg-amber-950/20">
              ...
              <AlertDescription className="flex items-center justify-between gap-4">
                ...
                <div className="flex gap-2 shrink-0">
                  <Button size="sm" variant="outline" onClick={restaurarRascunho} className="border-amber-400 text-amber-800 hover:bg-amber-100">
                    <RotateCcw className="w-3.5 h-3.5 mr-1" />
                    Continuar
                  </Button>
                  <Button size="sm" variant="ghost" onClick={descartarRascunho} className="text-amber-700 hover:bg-amber-100">
