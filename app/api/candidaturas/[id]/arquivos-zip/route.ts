...
type ArqFile = { caminho: string; nome: string; tipoCandidato?: string };
...
export async function GET(
  ...
  const arquivos: ArquivoEntry[] = [
    ...candidatura.arquivos.map((a: ArqFile) => ({ caminho: a.caminho, nome: a.nome, pasta: "inscricao" })),
    ...(candidatura.organizacao?.arquivos ?? ([] as ArqFile[])).map((a: ArqFile) => ({ caminho: a.caminho, nome: a.nome, pasta: "entidade" })),
    ...candidatura.candidatos.flatMap((c) =>
      c.arquivos.map((a: ArqFile) => ({ caminho: a.caminho, nome: a.nome, pasta: `candidatos/${c.tipoCandidato.toLowerCase()}` })),
    ),
  ];
