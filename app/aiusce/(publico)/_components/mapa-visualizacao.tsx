...
async function loadLayer(
  ...
  try {
    const kmlText = await fetch(`/api/aiusce/shapes/${name}`).then((r) => r.text());
...
const MapaVisualizacaoAiusce: React.FC<MapaVisualizacaoProps> = ({
  className = "",
  height = "400px",
}) => {
  ...
  useEffect(() => {
    ...
    const perimetroLayer = new VectorLayer({ source: new VectorSource() });
    const adesaoLayer = new VectorLayer({ source: new VectorSource() });

    const map = new Map({
      target: mapRef.current,
      layers: [new TileLayer({ source: new OSM() }), perimetroLayer, adesaoLayer],
      view: new View({
        center: fromLonLat([-46.637, -23.543]),
        zoom: 13,
      }),
    ...
    // Perímetro de adesão — shapes fundidos em área única
    loadLayer(
      adesaoLayer,
      "perimetro_adesao",
      new Style({
        fill: new Fill({ color: "rgba(16, 185, 129, 0.3)" }),
        stroke: new Stroke({ color: "rgba(16, 185, 129, 0.9)", width: 2 }),
      }),
      true
    );
    ...
    <div className={`relative w-full border border-border rounded-lg ${className}`} style={{ height }}>
      ...
      <div className="absolute bottom-3 left-3 z-10 bg-white/90 backdrop-blur-sm rounded-md shadow px-3 py-2 flex flex-col gap-1.5 text-xs">
        <div className="flex items-center gap-2">
          <span className="inline-block w-4 h-4 rounded-sm flex-shrink-0" style={{ background: "rgba(16, 185, 129, 0.3)", border: "2px solid rgba(16, 185, 129, 0.9)" }} />
...
export default MapaVisualizacaoAiusce;
