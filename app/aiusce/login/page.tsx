...
export default async function Login() {
	...
	if (session) {
		...
		if (session.user?.tipo === 'externo') {
			if ((session.user as any)?.primeiroAcesso) redirect('/aiusce/portal/alterar-senha');
			redirect('/aiusce/portal/minha-inscricao');
		}
		if (permissao && ['DEV', 'ADM'].includes(permissao)) redirect('/aiusce/candidaturas');
		// autenticado mas sem permissão de acesso — desloga
		redirect('/api/auth-aiusce/signout?callbackUrl=/aiusce/login');
	}
	return (
		<>
			<Background />
