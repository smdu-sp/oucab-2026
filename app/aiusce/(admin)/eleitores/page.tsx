...
import { buscarEleitoresAiusce, IAiusceEleitor, IAiusceEleitorPaginado } from "@/services/candidaturas-aiusce";
import { validaUsuarioAiusce } from "@/services/usuario-aiusce";

export default async function EleitoresAiusceSuspense({
  searchParams,
}: {
  ...
  return (
    <Suspense fallback={<TableSkeleton />}>
      <EleitoresAiusce searchParams={searchParams} />
    </Suspense>
  );
}

async function EleitoresAiusce({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
}) {
  ...
  let dados: IAiusceEleitor[] = [];

  const response = await buscarEleitoresAiusce(+pagina, +limite, busca as string, status as string);
  if (response) {
    const paginado = response as IAiusceEleitorPaginado;
  ...
  return (
    <div className="px-0 md:px-8 relative pb-20 md:pb-14 h-full container mx-auto">
      <h1 className="text-xl md:text-4xl font-bold">Eleitores AIUSCE</h1>
