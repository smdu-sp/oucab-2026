...
import { buscarUsuariosAiuvl, IUsuarioAiuvlPaginado } from "@/services/usuario-aiuvl";
import { validaUsuarioAiuvl } from "@/services/usuario-aiuvl";
import ModalUpdateAndCreate from "./_components/modal-update-create";
import type { Usuario } from "@/lib/generated/aiuvl";

export default async function UsuariosAiuvlSuspense({
  searchParams,
}: {
  ...
  return (
    <Suspense fallback={<TableSkeleton />}>
      <UsuariosAiuvl searchParams={searchParams} />
    </Suspense>
  );
}

async function UsuariosAiuvl({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
}) {
  ...
  const response = await buscarUsuariosAiuvl(+pagina, +limite, busca as string, status as string, permissao as string, tipo as string);
  if (response) {
    const paginado = response as IUsuarioAiuvlPaginado;
  ...
  return (
    <div className="px-0 md:px-8 relative pb-20 md:pb-14 h-full container mx-auto">
      <h1 className="text-xl md:text-4xl font-bold">Usuários AIU-VL</h1>
