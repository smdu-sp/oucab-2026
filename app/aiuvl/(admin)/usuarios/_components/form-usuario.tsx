...
export default function FormUsuario({
  ...
  async function onBuscarLogin(values: z.infer<typeof schemaLogin>) {
    const resp = await fetch(
      `${BASE_PATH}/api/aiuvl/usuarios/buscar-por-login/${values.login}`,
    );
    if (resp.status !== 200) {
  ...
  function onSubmitUsuario(values: z.infer<typeof schemaUsuario>) {
    startTransition(async () => {
      if (isUpdating && user?.id) {
        const resp = await fetch(`${BASE_PATH}/api/aiuvl/usuarios/${user.id}`, {
          method: "PATCH",
        ...
        const resp = await fetch(`${BASE_PATH}/api/aiuvl/usuarios`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      document.getElementById("close-dialog-aiuvl")?.click();
      ...
      <Form {...formUsuario}>
        <form onSubmit={formUsuario.handleSubmit(onSubmitUsuario)} className="space-y-4">
          ...
          <div className="flex gap-2 items-center justify-end">
            <DialogClose id="close-dialog-aiuvl" asChild>
