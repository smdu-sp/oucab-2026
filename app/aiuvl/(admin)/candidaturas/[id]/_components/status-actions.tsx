...
export function StatusActions({ id, statusAtual, isDev, oculto }: StatusActionsProps) {
  ...
  async function atualizar(novoStatus: string, motivoTexto?: string) {
    setLoading(true);
    try {
      const res = await fetch(`${BASE_PATH}/api/aiuvl/candidaturas/${id}/status`, {
        method: "PATCH",
  ...
  return (
    <>
      <div className="flex gap-2 flex-wrap">
        ...
        <Button variant="destructive" disabled={loading || statusAtual === "INDEFERIDO"} onClick={() => setDialogAberto(true)}>
