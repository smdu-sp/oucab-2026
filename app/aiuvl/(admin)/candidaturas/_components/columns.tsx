...
function OcultarButton({ id, oculto }: { id: string; oculto: boolean }) {
  ...
  async function toggle() {
    setLoading(true);
    try {
      const res = await fetch(`${BASE_PATH}/api/aiuvl/candidaturas/${id}/ocultar`, { method: "PATCH" });
...
export function createColumns(isDev: boolean): ColumnDef<IAiuvlCandidatura>[] {
  return [
    {
    ...
    },
    {
      ...
      cell: ({ row }) => row.original.organizacao?.segmento
        ? <EnumBadge info={getInfo(SEGMENTO_AIUVL_INFO, row.original.organizacao.segmento)} />
        : "—",
    },
    {
      accessorKey: "status",
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex justify-center gap-1">
          ...
          <BtnDownloadArquivos url={`${BASE_PATH}/api/aiuvl/candidaturas/${row.original.id}/arquivos-zip`} variant="icon" />
          <Button asChild size="icon" variant="ghost">
            <Link href={`/aiuvl/candidaturas/${row.original.id}`}>
