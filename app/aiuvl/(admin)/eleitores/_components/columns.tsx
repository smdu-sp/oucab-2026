...
function OcultarButton({ id, oculto }: { id: string; oculto: boolean }) {
  ...
  async function toggle() {
    setLoading(true);
    try {
      const res = await fetch(`${BASE_PATH}/api/aiuvl/eleitores/${id}/ocultar`, { method: "PATCH" });
...
export function createColumns(isDev: boolean): ColumnDef<IAiuvlEleitor>[] {
  return [
    {
      ...
      cell: ({ row }) => (
        <div className="flex justify-center gap-1">
          ...
          <Button asChild size="icon" variant="ghost">
            <Link href={`/aiuvl/eleitores/${row.original.id}`}>
