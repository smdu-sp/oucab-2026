...
function EnderecoForm({ prefix }: { prefix: Prefix }) {
  const { register, setValue, formState: { errors } } = useFormContext<FormularioAiuvlData>();
  ...
  const buscarCep = async (cep: string) => {
    ...
    try {
      ...
      if (data.erro) { setErroViaCep("CEP não encontrado."); return; }
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
        ...
        <div className="md:col-span-4 space-y-2">
          ...
          <Input {...register(`${prefix}.logradouro`)} placeholder="Rua, Avenida, etc." className={cn(e.logradouro && "border-red-500")} />
        ...
        <div className="md:col-span-2 space-y-2">
          <Label>Número *</Label>
          <Input {...register(`${prefix}.numero`)} placeholder="123" className={cn(e.numero && "border-red-500")} />
          {e.numero && <p className="text-sm text-red-500 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{e.numero.message}</p>}
        </div>

        <div className="md:col-span-4 space-y-2">
          <Label>Complemento</Label>
          <Input {...register(`${prefix}.complemento`)} placeholder="Sala, Andar, Bloco, etc. (opcional)" />
        </div>

        <div className="md:col-span-3 space-y-2">
          <Label>Bairro *</Label>
          <Input {...register(`${prefix}.bairro`)} placeholder="Bairro" className={cn(e.bairro && "border-red-500")} />
          {e.bairro && <p className="text-sm text-red-500 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{e.bairro.message}</p>}
        </div>

        <div className="md:col-span-2 space-y-2">
          <Label>Cidade *</Label>
          <Input {...register(`${prefix}.cidade`)} placeholder="São Paulo" className={cn(e.cidade && "border-red-500")} />
