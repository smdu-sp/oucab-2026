...
function CandidatoForm({ prefix }: { prefix: Prefix }) {
  ...
  const validarCPFRemoto = async (v: string) => {
    ...
    try {
      const res = await fetch(`/api/aiuvl/validacao/cpf/${cpf}`);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <Input
              ...
              onChange={(ev) => { setValue(`${prefix}.cpf` as any, formatarCPF(ev.target.value)); setCpfStatus(null); }}
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            className={cn(e.email && "border-red-500")}
          />
    ...
    <div className="space-y-5">
      ...
      <Alert className="border-blue-200 bg-blue-50 dark:bg-blue-950/30">
        ...
        <AlertDescription className="text-blue-800 dark:text-blue-200 text-xs">
          <strong>Paridade de gênero:</strong> ao menos um dos candidatos (titular ou suplente) deve ser do gênero feminino.
        </AlertDescription>
      </Alert>
...
export function EtapaDadosSuplente() {
  ...
  const titularGenero = watch("titular.genero" as keyof FormularioAiuvlData) as string | undefined;
  ...
  return (
    <div className="space-y-5">
      ...
      {titularGenero === "MASCULINO" && (
        <Alert className="border-amber-300 bg-amber-50 dark:bg-amber-950/30">
          <AlertCircle className="h-4 w-4 text-amber-600" />
          <AlertDescription className="text-amber-800 dark:text-amber-200 text-xs">
            <strong>Paridade de gênero obrigatória:</strong> o titular informado é do gênero masculino. O(a) suplente deve ser do gênero <strong>feminino</strong>.
          </AlertDescription>
        </Alert>
      )}

      {titularGenero === "FEMININO" && (
        <Alert className="border-emerald-300 bg-emerald-50 dark:bg-emerald-950/30">
          ...
          <AlertDescription className="text-emerald-800 dark:text-emerald-200 text-xs">
            O titular é do gênero feminino — o requisito de paridade de gênero já está atendido.
          </AlertDescription>
        </Alert>
      )}
