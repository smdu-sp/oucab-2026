...
CREATE TABLE `candidaturas` (
    ...
    `status` ENUM('EM_ANALISE', 'DEFERIDO', 'INDEFERIDO', 'AGUARDANDO_DOCUMENTACAO') NOT NULL DEFAULT 'EM_ANALISE',
    `oculto` BOOLEAN NOT NULL DEFAULT false,
    `motivoIndeferimento` TEXT NULL,
...
CREATE TABLE `organizacoes_candidatas` (
    ...
    `segmento` ENUM('ONG', 'ASSOCIACAO_BAIRRO', 'ENTIDADE_ACADEMICA', 'REP_EMPRESARIAL') NOT NULL,
...
CREATE TABLE `organizacoes_eleitoras` (
    ...
    `cnpj` VARCHAR(191) NOT NULL,
    `segmento` ENUM('ONG', 'ASSOCIACAO_BAIRRO', 'ENTIDADE_ACADEMICA', 'REP_EMPRESARIAL') NOT NULL,
    `segmentoVotacao` ENUM('ONG', 'ASSOCIACAO_BAIRRO', 'ENTIDADE_ACADEMICA', 'REP_EMPRESARIAL') NOT NULL,
    `dataAbertura` DATE NOT NULL,
    `sede` VARCHAR(191) NOT NULL,
    `repNome` VARCHAR(191) NOT NULL,
    `repCpf` VARCHAR(191) NOT NULL,
...
CREATE TABLE `arquivos` (
    ...
    `categoria` ENUM('CAND_ENT_REQUERIMENTO', 'CAND_ENT_DECLARACAO_ATUACAO', 'CAND_ENT_ESTATUTO', 'CAND_ENT_ATA_ELEICAO', 'CAND_ENT_CNPJ', 'CAND_ENT_DECLARACAO_IDONEIDADE', 'CAND_REP_IDENTIDADE', 'CAND_REP_CPF', 'CAND_REP_TITULO_ELEITOR', 'CAND_REP_COMPROVANTE_RESIDENCIA', 'CAND_CAN_IDENTIDADE', 'CAND_CAN_CPF', 'CAND_CAN_FOTO', 'CAND_CAN_TITULO_ELEITOR', 'CAND_CAN_COMPROVANTE_RESIDENCIA', 'CAND_CAN_DECLARACAO', 'ELEIT_ENT_REQUERIMENTO', 'ELEIT_ENT_DECLARACAO_ATUACAO', 'ELEIT_ENT_ESTATUTO', 'ELEIT_ENT_ATA_ELEICAO', 'ELEIT_ENT_CNPJ', 'ELEIT_ENT_DECLARACAO_IDONEIDADE', 'ELEIT_REP_IDENTIDADE', 'ELEIT_REP_CPF', 'ELEIT_REP_TITULO_ELEITOR', 'ELEIT_REP_COMPROVANTE_RESIDENCIA', 'COMPLEMENTAR') NOT NULL,
